...
```mermaid
graph LR
A[LinkedIn long-form 2-3 bai/tuan] --> B[Newsletter signup CTA]
B --> C[Email weekly insights]
C --> D{Lead score}
D -->|High intent| E[1-1 Discovery call free 30p]
D -->|Mid intent| F[Workshop 4tr]
E --> G[Advisory 50tr 3 thang]
F --> G
G --> H[Speaking gig referral]
```
...
```mermaid
graph LR
A[IG / FB content daily] --> B[Free guide PDF]
B --> C[Email nurture 7 ngay]
C --> D{Self-segment}
D -->|Beginner| E[Mini-course 1.5tr]
D -->|Mid level| F[Cohort 12tr]
D -->|Advanced| G[1-1 coaching 40tr]
E --> F
F --> G
G --> H[Retreat / Referral]
```
...
```mermaid
graph LR
A[TikTok / IG daily] --> B[Affiliate link bio]
A --> C[Brand deal pitch inbound]
A --> D[Email list signup]
D --> E[Own product launch]
E --> F[Membership / Patreon]
B --> F
C --> F
```
